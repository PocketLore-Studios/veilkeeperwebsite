...
just factsheet  # re-render the press factsheet PDF (needs a browser; not in ci)
```
...
A sitemap (`sitemap-index.xml`, via `@astrojs/sitemap`) and `public/robots.txt` are also emitted. `public/_headers` is interpreted by Cloudflare's static-asset serving (Pages-style syntax): baseline security headers on everything plus tiered cache-control (`/_astro/*` immutable, `/assets/*` 30 days, everything else - HTML, feeds - `max-age=0, must-revalidate` so pages never reference retired hashed assets). Site-wide constants (social URLs - the Discord invite especially, since invites get rotated - site URL, security email) live in `src/lib/site.ts`; never hardcode them in markup.
...
**Styling stays global CSS from the pre-Astro site:** five plain global stylesheets in `src/styles/`, imported in cascade order (`base`, `layout`, `components`, `sections`, `responsive`) by `BaseLayout.astro` so Astro bundles them into a content-hashed `/_astro/*.css` file (CSS edits bust caches; they used to ship stale from `public/css/`). No scoped Astro styles. Card-style elements share the `surface` class. Keep new markup compatible with these classes rather than adding scoped styles.
